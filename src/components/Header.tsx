    ...
    >
      <nav className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-8 text-md">
          ...
          <Link to="/chat" className="hover:text-blue-400 font-medium">Chat</Link>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden px-6 py-4 bg-gray-900 bg-opacity-90 space-y-4">
          ...
          <Link to="/chat" className="block hover:text-blue-400 font-medium" onClick={() => setIsMobileMenuOpen(false)}>Chat</Link>
        </div>
      )}
    </header>
  );
